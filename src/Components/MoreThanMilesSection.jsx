    ...
    <section ref={sectionRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
        {cards.map((item) => (
          <Card
            key={item.id}
            className="miles-card group relative overflow-hidden rounded-3xl border-0 p-0 shadow-sm transition-all duration-300 hover:shadow-xl cursor-pointer ring-0 aspect-[4/5]"
          >
            <img
              src={item.image}
              alt={item.title}
